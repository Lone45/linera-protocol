...
* `--external-protocol <EXTERNAL_PROTOCOL>` — External protocol used, either grpc or grpcs
  ...
  Default value: `grpc`
* `--with-faucet` — If present, a faucet is started using the chain provided by --faucet-chain, or the default chain if not provided

  Default value: `false`
* `--faucet-chain <FAUCET_CHAIN>` — If present, a faucet is started using the given chain root number (0 for the admin chain, 1 for the first non-admin initial chain, etc)
